...
# AESAlgorithm('aaaa', '22222')
# AESAlgorithm("1234","1234567890987654")


def aes_encryption_plaintext(plaintext, secret_key):
    result_aes = {}
    for k, v in change_text_cmp("keyskeyskeyskeys", plaintext, secret_key).items():
        result_aes[k] = v
    return result_aes


def aes_encryption_secret_key(plaintext, secret_key):
    result_aes = {}
    for k, v in change_key_cmp("GUETGUETGUET", plaintext, secret_key).items():
        result_aes[k] = v
    return result_aes


def aes_for_result(key, plaintext):
    result_aes = {}
    for k, v in change_text_cmp(key, plaintext, plaintext).items():
        result_aes[k] = v


args = sys.argv

if args[1] == "encryption_plaintext":
    print(aes_encryption_plaintext(*args[2:]))

if args[1] == "encryption_secret_key":
    print(aes_encryption_secret_key(*args[2:]))
